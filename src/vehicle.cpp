...
vector<string> Vehicle::successor_states() {
  ...
  //std::cout<<"Veh state is "<<this->state<<" lanes_available"<<lanes_available<<"\n";
  ...
  if (get_vehicle_ahead(predictions, lane, vehicle_ahead))
    {
        if (get_vehicle_behind(predictions, lane, vehicle_behind))
            ...
            /*if(vehicle_ahead.s-this->s > this->preferred_buffer+6)
                {
                    float max_velocity_in_front = (vehicle_ahead.s - this->s
                                  - this->preferred_buffer+6) + vehicle_ahead.v
                                  - 0.5 * (this->a);
                ...
                std::cout<<" lane veh in front"<< lane<<" v " << vehicle_ahead.v<<"\n";
                float max_velocity_in_front = (vehicle_ahead.s - this->s
               ...
               float max_velocity_in_front = (vehicle_ahead.s - this->s
                                  - this->preferred_buffer-10) + vehicle_ahead.v
                                  - 0.5 * (this->a);
                new_velocity = std::min(std::min(max_velocity_in_front,
                                       max_velocity_accel_limit),
                                      this->target_speed);
...
vector<Vehicle> Vehicle::lane_change_trajectory(string state,
  ...
  for (map<int, vector<Vehicle>>::iterator it = predictions.begin();
    ...
    if (
    ((next_lane_vehicle.s > this->s - (this->preferred_buffer) && next_lane_vehicle.s <= this->s)
    ||(next_lane_vehicle.s > this->s && next_lane_vehicle.s < this->s + (this->preferred_buffer+6))
    ||(next_lane_vehicle.s > this->s - (this->preferred_buffer+15) && next_lane_vehicle.s <= this->s && next_lane_vehicle.v > this->v)
    ||(next_lane_vehicle.s > this->s && next_lane_vehicle.s < this->s + (this->preferred_buffer+ 15) && next_lane_vehicle.v > this->v)
    )
...
void  Vehicle::print_vehicle_para()
{
std::cout<<this->lane<<"  "<<this->s<<"  "<<this->v<<"  "<<this->a<<"  "<<this->state<<" "<<this->max_acceleration<<"\n";
}
